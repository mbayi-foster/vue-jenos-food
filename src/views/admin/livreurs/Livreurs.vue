<template>
    <div class="mb-5">
        <BreadCumb hote="Livreurs" lien="#" page="Livreurs" :principale="true" />
    </div>
<TableLivreurs @effacer="effacer" @change-status="change" :data="users" :columns="tablesColumn" :has-data="hasData"
...
import TableLivreurs from '@/components/tables/TableLivreurs.vue'
...
const tablesColumn = ref([
    { key: 'nom', label: 'Livreurs' },
    { key: 'email', label: 'Contact' },
    { key: 'busy', label: 'Disponibilité' },
...
const fetchItems = async () => {
    try {
        load.value = true
        hasData.value = false
        const data = await api.get('/livreurs')
...
const change = async (id) => {
    try {
        await api.get(`/livreurs/status/${id}`)
        fetchItems()
    } catch (error) {
        console.log(error)
    }
